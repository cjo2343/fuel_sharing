...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.(?:214|215|216|217|218)"/, 'runtime version must be bumped for app.js changes.');
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v31[45678]"/, 'build-info must expect the bumped service-worker cache.');
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v31[45678]"/, 'service-worker cache must be bumped with runtime changes.');
