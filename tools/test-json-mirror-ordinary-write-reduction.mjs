...
assert.match(buildInfo, /version: "2026\.06\.18\.(?:215|216)"/, 'Runtime version should be bumped for app.js changes.');
assert.match(buildInfo, /expectedServiceWorkerCache: "fuel-ledger-v(?:315|316)"/, 'Expected service worker cache should be bumped.');
assert.match(serviceWorker, /fuel-ledger-v(?:315|316)/, 'Service worker cache should be bumped with build-info.');
