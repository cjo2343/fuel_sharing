...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.219"/, "runtime version should be bumped for admin tool dedupe runtime changes.");
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v319"/, "build-info should expect the bumped service-worker cache.");
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v319"/, "service worker cache should be bumped with runtime changes.");
