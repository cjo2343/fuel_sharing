...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.(?:204|205|206|207|208|209)"/, 'build-info should publish v304 or later');
...
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v30[4-9]"/, 'build-info should expect v304 or later cache');
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v30[4-9]"/, 'service worker cache should be v304 or later');
