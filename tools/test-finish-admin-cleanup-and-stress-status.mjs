...
assert.match(buildInfo, /finish-admin-cleanup-and-stress-status|normalized-test-data-cleanup|render-admin-test-data-routes|render-normalized-test-data-cleanup-route/, 'build label should preserve or build on the admin cleanup/stress finish patch');
assert.match(serviceWorker, /fuel-ledger-v29[2-5]/, 'service worker cache must stay bumped for runtime app.js changes');
