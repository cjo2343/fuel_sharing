...
assert.match(buildInfo, /finish-admin-cleanup-and-stress-status|normalized-test-data-cleanup|render-admin-test-data-routes|render-normalized-test-data-cleanup-route|render-cleanup-availability-hotfix|cleanup-scope-admin-finish-hotfix|admin-startup-flicker-reduction|startup-auth-hydration-screen|render-retention-admin-routes|admin-cleanup-diagnostics-finish|render-admin-health-endpoint|server-route-rate-limits|save-report-timeout-feedback|backend-path-simplification-audit|remove-proven-browser-fallbacks-pass-1|backend-path-simplification-audit|remove-proven-browser-fallbacks-pass-1|backend-path-simplification-audit|remove-proven-browser-fallbacks-pass-1/, 'build label should preserve or build on the admin cleanup/stress finish patch');
assert.match(serviceWorker, /fuel-ledger-v(?:29[2-9]|30[0-6])/, 'service worker cache must stay bumped for runtime app.js changes');
