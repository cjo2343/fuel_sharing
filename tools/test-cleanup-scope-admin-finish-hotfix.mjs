...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.(?:197|198|199|200|201|202|203|204|205|205|205|203|204|205|206)"/, "build-info.js must be bumped to v297.");
assert.match(buildInfo, /buildLabel:\s*"(?:cleanup-scope-admin-finish-hotfix|admin-startup-flicker-reduction|startup-auth-hydration-screen|render-retention-admin-routes|admin-cleanup-diagnostics-finish|render-admin-health-endpoint|server-route-rate-limits|save-report-timeout-feedback|backend-path-simplification-audit|remove-proven-browser-fallbacks-pass-1|backend-path-simplification-audit|remove-proven-browser-fallbacks-pass-1|backend-path-simplification-audit|remove-proven-browser-fallbacks-pass-1|backend-path-simplification-audit|remove-proven-browser-fallbacks-pass-1)"/, "build-info.js must use the v297 build label.");
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v(?:29[7-9]|30[0-6])"/, "build-info.js must expect the v297 service-worker cache.");
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v(?:29[7-9]|30[0-6])"/, "service-worker.js must use the v297 cache.");
assert.match(serviceWorker, /BUILD_LABEL\s*=\s*"(?:cleanup-scope-admin-finish-hotfix|admin-startup-flicker-reduction|startup-auth-hydration-screen|render-retention-admin-routes|admin-cleanup-diagnostics-finish|render-admin-health-endpoint|server-route-rate-limits|save-report-timeout-feedback|backend-path-simplification-audit|remove-proven-browser-fallbacks-pass-1|backend-path-simplification-audit|remove-proven-browser-fallbacks-pass-1|backend-path-simplification-audit|remove-proven-browser-fallbacks-pass-1|backend-path-simplification-audit|remove-proven-browser-fallbacks-pass-1)"/, "service-worker.js must use the v297 build label.");
...
assert.match(saveListener, /requireTypedAdminConfirmation/, "Save report confirmation must happen before opening the admin Data I/O row.");
