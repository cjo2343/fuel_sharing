...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.(?:197|198|199|200|201|202|203|204|205|206|207|208|209)"/, "build-info.js must be bumped to v297.");
...
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v(?:29[7-9]|30[0-9])"/, "build-info.js must expect the v297 service-worker cache.");
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v(?:29[7-9]|30[0-9])"/, "service-worker.js must use the v297 cache.");
