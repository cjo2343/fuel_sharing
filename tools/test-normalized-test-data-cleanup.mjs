...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.(?:19[3-9]|20[0-3])"/, "Build info must be bumped for v293.");
assert.match(buildInfo, /buildLabel:\s*"(?:normalized-test-data-cleanup|render-admin-test-data-routes|render-normalized-test-data-cleanup-route|render-cleanup-availability-hotfix|cleanup-scope-admin-finish-hotfix|admin-startup-flicker-reduction|startup-auth-hydration-screen|render-retention-admin-routes|admin-cleanup-diagnostics-finish|render-admin-health-endpoint|server-route-rate-limits)"/, "Build label must describe normalized Test Lab cleanup or a later build preserving it.");
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v(?:29[3-9]|30[0-3])"/, "Expected service-worker cache must be v293.");
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v(?:29[3-9]|30[0-3])"/, "Service-worker cache must be v293.");
assert.match(serviceWorker, /BUILD_LABEL\s*=\s*"(?:normalized-test-data-cleanup|render-admin-test-data-routes|render-normalized-test-data-cleanup-route|render-cleanup-availability-hotfix|cleanup-scope-admin-finish-hotfix|admin-startup-flicker-reduction|startup-auth-hydration-screen|render-retention-admin-routes|admin-cleanup-diagnostics-finish|render-admin-health-endpoint|server-route-rate-limits)"/, "Service-worker build label must be v293 or a later preserving label.");
