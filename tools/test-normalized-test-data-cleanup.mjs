...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.(?:19[3-9]|20[0-9])"/, "Build info must be bumped for v293.");
...
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v(?:29[3-9]|30[0-9])"/, "Expected service-worker cache must be v293.");
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v(?:29[3-9]|30[0-9])"/, "Service-worker cache must be v293.");
