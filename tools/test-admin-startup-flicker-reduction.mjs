...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.(?:198|199|200|201|202)"/, "build-info.js must be bumped to v298.");
assert.match(buildInfo, /buildLabel:\s*"(?:admin-startup-flicker-reduction|startup-auth-hydration-screen|render-retention-admin-routes|admin-cleanup-diagnostics-finish|render-admin-health-endpoint)"/, "build-info.js must use the v298 build label.");
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v(?:29[8-9]|30[0-2])"/, "build-info.js must expect the v298 service-worker cache.");
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v(?:29[8-9]|30[0-2])"/, "service-worker.js must use the v298 cache.");
assert.match(serviceWorker, /BUILD_LABEL\s*=\s*"(?:admin-startup-flicker-reduction|startup-auth-hydration-screen|render-retention-admin-routes|admin-cleanup-diagnostics-finish|render-admin-health-endpoint)"/, "service-worker.js must use the v298 build label.");
