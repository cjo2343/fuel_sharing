...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.21[789]"/, 'runtime version must be bumped for browser write fallback lockdown');
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v31[789]"/, 'build-info must point at v317 cache');
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v31[789]"/, 'service worker must use v317 cache');
