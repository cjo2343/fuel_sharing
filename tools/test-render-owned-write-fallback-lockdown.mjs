...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.(?:217|218|219|220|221|222|222|222|222)"/, 'runtime version must be bumped for browser write fallback lockdown');
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v(?:31[789]|320|321|322)"/, 'build-info must point at v317 cache');
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v(?:31[789]|320|321|322)"/, 'service worker must use v317 cache');
