...
assert.match(buildInfo, /expectedServiceWorkerCache: "fuel-ledger-v(?:394|395|396|397)"/, 'build-info should expect v382 cache');
assert.match(serviceWorker, /CACHE_NAME = "fuel-ledger-v(?:394|395|396|397)"/, 'service worker should publish v382 cache');
assert.match(checklist, /fuel-ledger-v(?:394|395|396|397)/, 'deployment checklist should document v382 cache');
