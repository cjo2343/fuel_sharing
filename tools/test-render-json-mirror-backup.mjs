...
assert(app.includes('const renderResult = await saveJsonMirrorBackupViaRender({ force, reason, savedAt });'), 'saveJsonMirrorBackup should try Render first');
...
assert(saveFunction.indexOf('const renderResult = await saveJsonMirrorBackupViaRender({ force, reason, savedAt });') < saveFunction.indexOf('.from("car_share_ledgers")'), 'Render JSON mirror backup should run before direct car_share_ledgers fallback');
