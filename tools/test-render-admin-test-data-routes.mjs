...
assert(/version:\s*"2026\.06\.18\.(?:19[4-9]|20[0-8])"/.test(buildInfo), 'build-info.js must be v294 or later while preserving the admin test-data routes.');
...
assert(/expectedServiceWorkerCache:\s*"fuel-ledger-v(?:29[4-9]|30[0-8])"/.test(buildInfo), 'build-info.js must expect the v294/v295/v296/v297 service-worker cache.');
assert(/CACHE_NAME\s*=\s*"fuel-ledger-v(?:29[4-9]|30[0-8])"/.test(serviceWorker), 'service-worker.js must use the v294/v295/v296/v297 cache.');
