...
assert.match(buildInfo, /buildLabel:\s*"(?:interactive-action-recovery-lane|write-context-fast-fail-lane|no-refresh-action-chain-lane|no-refresh-action-chain-lane|post-action-unblock-lane|multi-workspace-authority-lane)"/, 'build-info should use the multi-workspace authority label');
assert.match(serviceWorker, /BUILD_LABEL\s*=\s*"(?:interactive-action-recovery-lane|write-context-fast-fail-lane|no-refresh-action-chain-lane|no-refresh-action-chain-lane|post-action-unblock-lane|multi-workspace-authority-lane)"/, 'service worker should use the multi-workspace authority label');
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"(?:fuel-ledger-v407|fuel-ledger-v406|fuel-ledger-v405|fuel-ledger-v404|fuel-ledger-v403|fuel-ledger-v402)"/, 'service worker cache should be v402');
