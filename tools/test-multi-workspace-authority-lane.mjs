...
assert.match(buildInfo, /buildLabel:\s*"multi-workspace-authority-lane"/, 'build-info should use the multi-workspace authority label');
assert.match(serviceWorker, /BUILD_LABEL\s*=\s*"multi-workspace-authority-lane"/, 'service worker should use the multi-workspace authority label');
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v401"/, 'service worker cache should be v401');
