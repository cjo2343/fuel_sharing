...
assert.match(buildInfo, /buildLabel:\s*"(?:post-action-unblock-lane|multi-workspace-authority-lane)"/, 'build-info should use the multi-workspace authority label');
assert.match(serviceWorker, /BUILD_LABEL\s*=\s*"(?:post-action-unblock-lane|multi-workspace-authority-lane)"/, 'service worker should use the multi-workspace authority label');
