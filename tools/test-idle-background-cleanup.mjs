...
assert.match(buildInfo, /expectedServiceWorkerCache: "fuel-ledger-v390"/, 'build-info should expect v386 cache');
assert.match(serviceWorker, /CACHE_NAME = "fuel-ledger-v390"/, 'service worker should publish v386 cache');
assert.match(checklist, /fuel-ledger-v390/, 'deployment checklist should document v386 cache');
