...
assert.match(buildInfo, /expectedServiceWorkerCache: "fuel-ledger-v391"/, 'build-info should expect v386 cache');
assert.match(serviceWorker, /CACHE_NAME = "fuel-ledger-v391"/, 'service worker should publish v386 cache');
assert.match(checklist, /fuel-ledger-v391/, 'deployment checklist should document v386 cache');
