...
assert.match(buildInfo, /expectedServiceWorkerCache: "fuel-ledger-v393"/, 'build-info should expect v386 cache');
assert.match(serviceWorker, /CACHE_NAME = "fuel-ledger-v393"/, 'service worker should publish v386 cache');
assert.match(checklist, /fuel-ledger-v393/, 'deployment checklist should document v386 cache');
