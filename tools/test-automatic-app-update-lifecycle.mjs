...
assert.match(buildInfo, /setInterval\(\(\) => \{[\s\S]*refreshBuildInfo\(\{ activateUpdates: true \}\)/, 'automatic version poll should refresh and activate updates');
...
assert.match(buildInfo, /expectedServiceWorkerCache: "fuel-ledger-v380"/, 'build-info should expect v379 cache');
assert.match(serviceWorker, /CACHE_NAME = "fuel-ledger-v380"/, 'service worker should publish v379 cache');
assert.match(checklist, /fuel-ledger-v380/, 'deployment checklist should document v379 cache');
