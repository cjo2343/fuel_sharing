...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.(?:206|207|208)"/, 'build-info should publish v306');
...
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v30[6-8]"/, 'build-info should expect v306 cache');
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v30[6-8]"/, 'service worker should use v306 cache');
