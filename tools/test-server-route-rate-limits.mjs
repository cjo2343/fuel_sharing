...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.(?:203|204|205|206|207|208|209)"/, 'build-info should publish v303');
...
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v30[3-9]"/, 'build-info should expect v303 cache');
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v30[3-9]"/, 'service worker cache should be v303');
