...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.(?:196|197|198|199|200|201|202|203|204|205|206|207)"/, "build-info.js must be v296 or a later preserving hotfix.");
...
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v(?:29[6-9]|30[0-7])"/, "build-info.js must expect the v296/v297 service-worker cache.");
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v(?:29[6-9]|30[0-7])"/, "service-worker.js must use the v296/v297 cache.");
