...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.(?:196|197|198|199|200|201|202)"/, "build-info.js must be v296 or a later preserving hotfix.");
assert.match(buildInfo, /buildLabel:\s*"(?:render-cleanup-availability-hotfix|cleanup-scope-admin-finish-hotfix|admin-startup-flicker-reduction|startup-auth-hydration-screen|render-retention-admin-routes|admin-cleanup-diagnostics-finish|render-admin-health-endpoint)"/, "build-info.js must use the v296/v297 hotfix build label.");
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v(?:29[6-9]|30[0-2])"/, "build-info.js must expect the v296/v297 service-worker cache.");
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v(?:29[6-9]|30[0-2])"/, "service-worker.js must use the v296/v297 cache.");
assert.match(serviceWorker, /BUILD_LABEL\s*=\s*"(?:render-cleanup-availability-hotfix|cleanup-scope-admin-finish-hotfix|admin-startup-flicker-reduction|startup-auth-hydration-screen|render-retention-admin-routes|admin-cleanup-diagnostics-finish|render-admin-health-endpoint)"/, "service-worker.js must use the v296/v297 hotfix build label.");
