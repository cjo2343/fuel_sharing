...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.(?:195|196|197|198|199|200)"/, "build-info.js must be v295 or a later preserving hotfix.");
...
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v(?:29[5-9]|300)"/, "build-info.js must expect the v295/v296/v297 service-worker cache.");
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v(?:29[5-9]|300)"/, "service-worker.js must use the v295/v296/v297 cache.");
assert.match(serviceWorker, /BUILD_LABEL\s*=\s*"(?:render-normalized-test-data-cleanup-route|render-cleanup-availability-hotfix|cleanup-scope-admin-finish-hotfix|admin-startup-flicker-reduction|startup-auth-hydration-screen|render-retention-admin-routes)"/, "service-worker.js must use the v295 build label or the v296 hotfix label.");
