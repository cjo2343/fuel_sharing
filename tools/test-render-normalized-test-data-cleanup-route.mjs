...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.195"/, "build-info.js must be bumped to v295.");
assert.match(buildInfo, /buildLabel:\s*"render-normalized-test-data-cleanup-route"/, "build-info.js must use the v295 build label.");
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v295"/, "build-info.js must expect the v295 service-worker cache.");
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v295"/, "service-worker.js must use the v295 cache.");
assert.match(serviceWorker, /BUILD_LABEL\s*=\s*"render-normalized-test-data-cleanup-route"/, "service-worker.js must use the v295 build label.");
