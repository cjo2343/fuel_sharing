...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.(?:195|196)"/, "build-info.js must be v295 or a later preserving hotfix.");
assert.match(buildInfo, /buildLabel:\s*"(?:render-normalized-test-data-cleanup-route|render-cleanup-availability-hotfix)"/, "build-info.js must use the v295 build label or the v296 hotfix label.");
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v29[56]"/, "build-info.js must expect the v295/v296 service-worker cache.");
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v29[56]"/, "service-worker.js must use the v295/v296 cache.");
assert.match(serviceWorker, /BUILD_LABEL\s*=\s*"(?:render-normalized-test-data-cleanup-route|render-cleanup-availability-hotfix)"/, "service-worker.js must use the v295 build label or the v296 hotfix label.");
