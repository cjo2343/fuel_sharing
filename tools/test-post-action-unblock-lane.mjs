...
assert.match(buildInfo, /buildLabel:\s*"(?:write-context-fast-fail-lane|no-refresh-action-chain-lane|post-action-unblock-lane|no-refresh-action-chain-lane)"/, 'build-info should use post-action unblock label');
assert.match(serviceWorker, /BUILD_LABEL\s*=\s*"(?:write-context-fast-fail-lane|no-refresh-action-chain-lane|post-action-unblock-lane|no-refresh-action-chain-lane)"/, 'service worker should use post-action unblock label');
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"(?:fuel-ledger-v406|fuel-ledger-v405|fuel-ledger-v404|fuel-ledger-v403|fuel-ledger-v402)"/, 'service worker cache should be v402');
