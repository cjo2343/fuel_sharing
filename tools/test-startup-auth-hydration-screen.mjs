...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.(?:199|200|201|202|203|204|205|206|207)"/, "build-info.js must be bumped to v299.");
...
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v(?:299|30[0-7])"/, "build-info.js must expect the v299 service-worker cache.");
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v(?:299|30[0-7])"/, "service-worker.js must use the v299 cache.");
