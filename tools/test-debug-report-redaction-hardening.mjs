...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.218"/, "runtime version should be bumped for redaction hardening");
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v318"/, "build-info should point at v318 cache");
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v318"/, "service worker should use v318 cache");
