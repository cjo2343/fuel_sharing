...
assert(app.includes('reason: `sync-lane:${String(reason || "sync")}`'), 'sync lane context requests should be labeled as sync-lane');
assert(app.includes('await ensureBackendAppContextForSyncLane(reason).catch'), 'loadSupabaseState should ensure backend context before loading state');
...
assert(app.includes('await ensureBackendAppContextForSyncLane("initial-session")'), 'initial session should use backend context as the first sync lane step');
assert(app.includes('await ensureBackendAppContextForSyncLane(`auth-${String(event || "session").toLowerCase()}`)'), 'auth session should use backend context as the first sync lane step');
