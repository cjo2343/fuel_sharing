...
function testReleaseAboutPanelExists() {
  ...
  assert.match(buildInfo, /scheduleBuildInfoRefresh\(\{ activateUpdates: false \}\)/);
...
function testAuthBoundWorkspaceIdentityExists() {
  ...
  assert.match(app, /hydrateAppSessionContext\(\{ reason: "initial-session", source: "startup" \}\)|ensureBackendAppContextForSyncLane\("initial-session"\)|ensureAppStartupWakeGate\("initial-session", \{ force: true \}\)|ensureAppStartupWakeGate\("initial-session", \{ force: true \}\)|refreshLinkedWorkspacesAfterInvite\(\)\.catch/);
...
function testWorkspaceInviteAutoRefreshExists() {
  ...
  assert.match(app, /if \(activeView === "admin"\) \{\s*startAdminDiagnosticsLane\("admin-tab-open"\);/);
  assert.match(app, /function startAdminDiagnosticsLane[\s\S]*markOptionalAdminPanelsReadyForManualRefresh\(reason\)/);
  assert.doesNotMatch(app, /scheduleWorkspaceInviteRefresh\("startup-session"\)/);
  assert.match(app, /hydrateAppSessionContext\(\{ reason: "initial-session", source: "startup" \}\)|ensureBackendAppContextForSyncLane\("initial-session"\)|ensureAppStartupWakeGate\("initial-session", \{ force: true \}\)/);
  assert.doesNotMatch(app, /scheduleWorkspaceInviteRefresh\("auth-session"\)/);
  assert.match(app, /hydrateAppSessionContext\(\{ reason: `auth-\$\{String\(event \|\| "session"\)\.toLowerCase\(\)\}`, source: "auth" \}\)|ensureBackendAppContextForSyncLane\(`auth-\$\{String\(event \|\| "session"\)\.toLowerCase\(\)\}`\)|ensureAppStartupWakeGate\(`auth-\$\{authEvent\}`, \{ force: !lastCloudSyncAt \}\)/);
...
function testDataIoFlightRecorderExists() {
  ...
  assert.match(app, /function hasActiveDataIoOperation\(referenceTime = Date\.now\(\), maxAgeMs = dataIoOperationStaleMs, options = \{\}\)/);
