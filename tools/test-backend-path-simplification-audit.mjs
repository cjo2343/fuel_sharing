...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.(?:205|206|207|208)"/, 'build-info should publish v305');
...
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v(?:305|306|307|308)"/, 'build-info should expect v305 cache');
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v(?:305|306|307|308)"/, 'service worker cache should be v305');
