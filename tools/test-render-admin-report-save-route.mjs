...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.207"/, 'build-info should publish v307');
assert.match(buildInfo, /buildLabel:\s*"render-admin-report-save-route"/, 'build-info should use v307 label');
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v307"/, 'build-info should expect v307 cache');
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v307"/, 'service worker should use v307 cache');
