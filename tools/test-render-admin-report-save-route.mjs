...
assert.match(buildInfo, /version:\s*"2026\.06\.18\.(?:207|208)"/, 'build-info should publish v307 or a preserving v308 patch');
assert.match(buildInfo, /buildLabel:\s*"render-admin-report-save-route"/, 'build-info should use v307 label');
assert.match(buildInfo, /expectedServiceWorkerCache:\s*"fuel-ledger-v30[78]"/, 'build-info should expect v307 or a preserving v308 cache');
assert.match(serviceWorker, /CACHE_NAME\s*=\s*"fuel-ledger-v30[78]"/, 'service worker should use v307 or a preserving v308 cache');
