...
assert.match(buildInfo, /buildLabel:\s*"(?:app-session-hydrate-lane|render-admin-report-save-route)"/, 'build-info should use v307 label');
...
assert.match(serviceWorker, /BUILD_LABEL\s*=\s*"(?:app-session-hydrate-lane|render-admin-report-save-route)"/, 'service worker label should match v307');
