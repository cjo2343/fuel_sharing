...
assert.match(buildInfo, /version: "2026\.06\.18\.(?:200|201|202)"/);
assert.match(buildInfo, /buildLabel: "(?:render-retention-admin-routes|admin-cleanup-diagnostics-finish|render-admin-health-endpoint)"/);
assert.match(buildInfo, /expectedServiceWorkerCache: "fuel-ledger-v(?:300|301|302)"/);
assert.match(serviceWorker, /CACHE_NAME = "fuel-ledger-v(?:300|301|302)"/);
assert.match(serviceWorker, /BUILD_LABEL = "(?:render-retention-admin-routes|admin-cleanup-diagnostics-finish|render-admin-health-endpoint)"/);
