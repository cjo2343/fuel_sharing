-- Migration 056: In-app account deletion — anonymize-and-scrub + auth user delete (GVM-115)
--
-- App Store guideline 5.1.1(v) requires in-app account deletion; GDPR requires
-- honouring erasure. Hard-deleting rows is not an option in a shared-money ledger:
-- the departing member's trips and fuel payments feed OTHER members' settlements
-- and the fingerprint-guarded closed-period snapshots (migration 054). So deletion
-- is anonymize-and-scrub:
--
--   * The member row survives (FKs from trips/fuel/settlements stay valid) but is
--     renamed to a per-ledger-unique 'Slettet medlem [n]' (unique(ledger_id, name))
--     with email/MobilePay nulled and is_active=false.
--   * Personal data is scrubbed: authored trip notes, fuel-stop user coordinates,
--     booking purposes (+ future bookings cancelled), chat messages (blanked +
--     soft-deleted), event actor/target emails and the member's name inside
--     authored/targeted event titles (events also expire after 30 days), the
--     member's name inside archived snapshot_json people/settlements, invite
--     emails, push subscriptions, onboarding rate-limit rows, and operator-log
--     actor identifiers.
--   * Financial facts are retained deliberately — km, amounts, dates, settlement
--     requests, repairs. Legal basis: the co-members' legitimate interest in
--     accurate shared accounts (GDPR art. 6(1)(f); art. 17(3) balancing). They no
--     longer reference an identifiable person once the scrub completes.
--   * If the caller is the only ACTIVE admin and other active members remain, the
--     longest-standing active member is promoted (with a feed event) so the
--     workspace never becomes admin-less — Apple rejects deletion flows that can
--     dead-end on "ask someone else first".
--   * If no other active member remains, the whole ledger is deleted outright
--     (every child table cascades on ledger delete) — nobody could access it again.
--   * Finally the auth.users row is deleted (cascades sessions/identities), so the
--     credentials die and the email is free to register a fresh account.
--
-- Scrub updates deliberately touch only non-settlement columns, so migration 046's
-- entry lock (which guards km/amount/date/member/period/deleted_at changes on
-- locked periods) never fires; car_bookings and messages are outside that trigger.
--
-- The RPC is strictly self-service: it acts on the caller's JWT identity only.
-- Operator-initiated erasure (user cannot sign in) stays a manual runbook.
