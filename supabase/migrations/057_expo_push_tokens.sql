...
--
-- Push delivery backend, part 1 of 3 (this migration; the sender is a
-- Cloudflare Pages Function in govehlo-web at /api/hooks/ledger-event; the
-- mobile app uploads its token after sign-in).
--
--   * expo_push_tokens: one row per signed-in device. Keyed by BOTH the auth
--     user id and the lowercased email — the sender resolves recipients from
--     ledger_members.email (the app's identity link), while account deletion
--     purges by user id. A token is unique across users: a device that signs
--     into a different account re-points its row (on conflict update), so a
--     shared phone never receives the previous user's notifications.
--   * RLS: owners see and manage only their own rows; writes go through the
--     RPCs below (validated + derived identity). The webhook sender reads with
--     the service role, which bypasses RLS.
--   * ledger_events.push_sent_at: the sender's idempotency claim. Supabase
--     Database Webhooks retry on failure, so the Function atomically claims
--     the event (update ... where push_sent_at is null) before sending —
--     a retry that lost the claim exits without duplicate pushes.
--   * delete_my_account is re-declared verbatim from migration 056 plus one
--     block: purge the departing user's push tokens (GDPR — a deleted account
--     must stop being addressable).
--
-- After applying: create the Database Webhook in the Supabase dashboard
-- (table public.ledger_events, INSERT, POST https://govehlo.dk/api/hooks/ledger-event,
-- header x-webhook-secret = the value set as LEDGER_EVENT_WEBHOOK_SECRET in
-- Cloudflare Pages) — documented in the govehlo-web PR.
