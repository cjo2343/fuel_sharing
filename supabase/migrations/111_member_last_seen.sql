-- Migration 111: persisted member last-seen (GVM-66)
--
-- GVM-28 shipped live presence via Supabase Realtime, but that presence is
-- ephemeral: the moment a member closes the app their dot goes grey and there is
-- no way to say how long ago they were around. This migration persists a single
-- last-seen timestamp per membership so offline members can show "Aktiv for 5 min.
-- siden" instead of a bare grey dot.
--
-- The heartbeat that writes it (touch_member_presence) is deliberately EVENT-FREE:
-- it must never insert into ledger_events, because the mobile client fires it on
-- every presence subscribe, foreground, and ~4-minute tick, and lighting up feeds
-- or realtime on every tick would be noise. It only bumps the caller's own row.
