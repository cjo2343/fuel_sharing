...
- `141_close_guard_retention_and_period_lockdown.sql` fixes three settlement-period defects: closing a period no longer trips migration 140's repair guard on its own legacy-repair cleanup write (GV-358), `enforce_repair_period_lock` regains migration 131's row-age retention carve-out that 140 dropped by re-declaring off the wrong ancestor (GV-359), and the never-dropped `Ledger admins can update periods` policy on `settlement_periods` is removed together with the client write grants (GV-351).
