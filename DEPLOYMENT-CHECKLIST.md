- 2026-06-24T13:00:00.000Z — `2026.06.18.297` / `sign-in-flow-review` / `fuel-ledger-v438`: Sign-in flow polish. Corrected stale "ask an admin to add your email" guidance across six screens to invite-only wording (ask a workspace admin for an invite, then redeem it). Tightened the login card copy to distinguish the workspace invite code (permission to join) from the emailed login code (proves you own the address). A signed-out visitor's URL no longer carries a workspace slug, so the logged-out page does not reveal which workspaces exist; the workspace is restored from membership after sign-in.
- 2026-06-24T12:00:00.000Z — `2026.06.18.296` / `smart-predictions-rules-audit` / `fuel-ledger-v437`: Smart predictions are more honest about fuel consumption. Historical L/100 km is now measured full-tank to full-tank (the liters burned between two brim fills over the odometer distance) instead of total-liters over total-km, which mixed in the first fill and partial fills and could wrongly distrust or trust the data. When there is no full-tank pair yet it falls back to the rough total-over-total estimate and labels it as such. With no liters logged the app now says not enough liters logged yet instead of claiming consumption looks plausible and quietly awarding confidence for data it does not have. The Planning source card no longer mislabels the price basis. When historical consumption is trusted and full-tank-measured, the Tank range now agrees with the Fuel intelligence panel; the trip-save overfill guard stays on the conservative car setting.
- 2026-06-24T09:00:00.000Z — `2026.06.18.295` / `member-invite-only-onboarding` / `fuel-ledger-v436`: Adding members is now invite-only. The direct Add-member-by-email form is gone; to bring someone in, create an invite under Account, Workspaces and invites, and share the code or link so they join by redeeming it (which confirms they own the email and respects invite expiry and use limits). Member management still lets workspace admins edit an existing member email, role, and MobilePay or deactivate old and test members. Both the Render member API and the upsert_ledger_member_admin database function now reject creating a brand-new member for everyone, including the app owner, so a redeemed invite is the single onboarding path even if the API is called directly.
- 2026-06-23T16:00:00.000Z — `2026.06.18.294` / `render-health-real-probes` / `fuel-ledger-v435`: The Render admin health report now actually reports health instead of showing a wall of green. It runs a real, short-timeout Supabase reachability probe (with latency) so a backend or RLS outage turns the row red and the overall status to needs-review, replaces twenty hardcoded always-green route rows with one honest dependency-gated row for the critical write/admin lanes, and reports the vehicle lookup provider from its last cached outcome rather than calling the external provider on every check. New runtime signals (process uptime, deployed app version, Supabase probe latency) surface free-tier cold starts and crash-loops, and the card now leads with any issues and collapses the passing checks behind an expander.
...
- Version: `2026.06.30.299`
- Service-worker cache: `fuel-ledger-v440`
- Updated at: `2026-06-30T12:00:00.000Z`
- Top release note: Render has been decommissioned. Its last consumer, the native app's owner-activity logging, now runs as a Cloudflare Pages Function (POST /api/mobile/activity on the public origin) and the app points there instead of the old Render host, so the stack is Cloudflare Pages plus Supabase only. This source-side cleanup removes the hard-coded personal-email fallback from the app-owner allow-list so a missing or empty APP_OWNER_EMAILS configuration authorizes nobody (fail-closed) instead of silently trusting a built-in address, matching the Cloudflare owner gate and keeping a personal address out of source.
- Previous top release note: Added a service-role-mediated owner-activity endpoint (POST /api/mobile/activity) so the native app can record safe, redacted owner-activity rows for meaningful actions. The native client writes to Supabase directly with the anon key plus a user JWT, but the owner activity log is service-role only by design, so the app physically cannot insert those rows itself. The new route authenticates the caller's Supabase JWT, confirms they are an active member of the target ledger, then records the action through the existing service-role helper, redacting metadata (no tokens, cookies, secrets, VINs, or raw bodies). The owner Workspaces and Errors views populate from these rows.
- Older top release note: Sign-in flow polish. Several screens still told users whose email was not a member to ask an admin to add it, which contradicted the invite-only onboarding (admins can no longer add members directly) — they now say to ask a workspace admin for an invite and redeem it to join. The login card copy is tightened to distinguish the workspace invite code (your permission to join a workspace) from the login code we email you next (which proves you own the address). And a signed-out visitor's URL no longer carries a workspace slug: opening a workspace deep-link while logged out now shows a clean URL so the logged-out page does not reveal which workspaces exist; after signing in, the workspace is restored from your membership. Historical L/100 km is now measured full-tank to full-tank (the liters burned between two brim fills over the odometer distance) instead of total-liters over total-km, which mixed in the first fill and partial fills and could wrongly distrust or trust the data. When there is no full-tank pair yet it falls back to the rough total-over-total estimate and labels it as such. With no liters logged the app now says not enough liters logged yet instead of claiming consumption looks plausible and quietly awarding confidence for data it does not have. The Planning source card no longer mislabels the price basis (it names a historical receipt average, live reference price, or fallback to match what the estimate actually uses). When historical consumption is trusted and full-tank-measured, the Tank range now agrees with the Fuel intelligence panel and states whether it is using your measured or the car-setting consumption; the trip-save overfill guard stays on the conservative car setting.
