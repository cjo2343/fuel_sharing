...
### Supabase Preview check 502 troubleshooting

The GitHub status named **Supabase Preview** is created by the Supabase GitHub App, not by the Fuel Ledger validation workflow. If **Validate Fuel Ledger / validate (push)** is green but **Supabase Preview** fails quickly with `Failed to create Preview Branch: unexpected status 502: error code: 502`, treat it as a Supabase integration/preview-branch problem rather than an app test failure.

Before changing app code, verify the Supabase project GitHub integration settings:

- The connected repository is `cjo2343/fuel_sharing`.
- The working directory points to the folder that contains the repo's `supabase/` folder; for this repo layout, `.` is correct.
- Production deploy remains pointed at `main`.
- Preview Branches / Branching is disabled unless the Supabase plan supports it.

If the failing status persists, re-run the Supabase check once. If it still fails with only the 502 preview-branch message, disconnect/reconnect the Supabase GitHub integration or disable preview-branch checks. Do not treat the failed Supabase Preview status as a failed app release when the Fuel Ledger GitHub Actions validation passed.
