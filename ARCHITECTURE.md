...
| Onboarding / invites | `renderWorkspaceInvitesPanel`, server invite RPCs; **invite-only** — the direct add-member form is removed and `upsert_member_as_user` (Render) + `upsert_ledger_member_admin` (SQL, migration 042) reject creating new members, so `redeem_ledger_invite` is the single onboarding path. `renderMemberManagementPanel` edits/deactivates existing members only | #5 done, #10 |
