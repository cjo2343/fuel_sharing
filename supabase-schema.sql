...
-- ── Migration 089: settlement status on a closed period (GVM-243) ──────────────
-- Mirror of supabase/migrations/089_settlement_status_on_closed_period.sql.
-- Last definition wins on replay; must match the migration byte-for-byte (GV-175).
...
-- ══════════════════════════════════════════════════════════════════════════
-- Migration 090: two-step settlement confirmation — paid_pending (GVM-213)
-- Mirror of supabase/migrations/090_settlement_paid_pending.sql (function
...
-- Mirror of supabase/migrations/091_reminder_activity_events.sql (function
-- bodies byte-identical for the GV-175 equivalence check).
-- ══════════════════════════════════════════════════════════════════════════
...
-- Mirror of supabase/migrations/092_tank_baseline.sql (function body
-- byte-identical for the GV-175 equivalence check).
-- ══════════════════════════════════════════════════════════════════════════
...
-- Home announcement banner (GVM-120) served via /app-config.json; writes are
-- service-role only (admin console), authenticated clients read active rows only.
...
-- ── Per-user notification preferences (095_notification_preferences.sql) ──────
-- Verbatim from 095_notification_preferences.sql: per-event-type push toggles,
...
-- Verbatim from 096_purge_notification_preferences_on_delete.sql: re-declare
-- delete_my_account (off migration 072) + a notification_preferences purge.
create or replace function public.delete_my_account()
returns jsonb
language plpgsql
security definer
...
-- Migration 098: per-workspace operator override for the settlement safety rules
-- (GV-158). Two boolean flags on ledgers (default true = current enforced
-- behaviour); enforce_settlement_entry_lock consults rule_lock_period_after_payment.
alter table public.ledgers
...
-- Migration 099: RLS security fixes (GV-243). Drop the forgeable settlement_periods
-- and ledger_events insert policies (no client inserts either table directly; all
-- legitimate writes are SECURITY DEFINER, so RLS default-deny is safe). Re-declare
-- enforce_settlement_entry_lock so the per-workspace override relaxes only the
-- requested/paid lock — the closed-period rejection is always enforced. These blocks
-- win on replay over the original 005 policy creations above.
drop policy if exists "Ledger members can insert periods" on public.settlement_periods;
...
-- ── Migration 100 mirror: fix check_owner_rate_limit 42702 ambiguity (GV-252) ──
-- Re-declares the limiter with #variable_conflict use_column; the actor_email
-- parameter shadowed the column in the ON CONFLICT target, so the RPC threw on
-- every call since 049. Last definition wins on replay.
...
-- ── Migration 101 mirror: reassignment protection + close invariants (GV-253) ──
-- FIX 1: one shared BEFORE UPDATE trigger lets only a ledger admin or the row
-- creator change the identity-bearing member column on trips/fuel_payments/
-- car_bookings, covering both the SECURITY DEFINER upsert RPCs and direct updates.
-- FIX 2: close_settlement_period (re-declared off 087) enforces
-- rule_require_requests_before_close. FIX 3: the new open period is labelled in
-- Danish ('Juli 2026') instead of English 'Current period'. Last definition wins.
...
-- ── Migration 103 mirror: settlement/join-code/bootstrap/identity hardening (GV-244..GV-247) ──
-- GV-244: upsert_settlement_request_status (re-declared off 090) gains a
--   party-or-admin guard so only a party or a ledger admin may create/transition
--   a settlement request. GV-245: generate_workspace_join_code (off 045) uses a
--   6-char body; the data-only join-code regeneration is intentionally NOT mirrored
--   (the equivalence check compares schema objects, not data). GV-246:
--   is_ledger_bootstrap_open removed — 3 bootstrap policies dropped, 5 policies
--   re-created without the bootstrap disjunct, then the function dropped (bootstrap
--   trigger + column kept). GV-247: enforce_identity_reassignment (off 101) also
--   requires the identity party to be an active member; triggers recreated as
--   before insert or update. Last definition wins on replay.
...
-- ── Migration 104: settlement amount integrity (GV-259, GV-260) ─────────────
...
-- ══════════════════════════════════════════════════════════════════════════
-- Mirror of supabase/migrations/106_reminder_outcomes_claim_token.sql (GV-265;
-- function bodies byte-identical for the GV-175 equivalence check). Adds a
-- per-lease claim token to both claim RPCs and moves the confirm RPCs to a
-- (confirmations jsonb) = [{"id","token","outcome"}] signature that records the
-- delivery outcome in the ledger_events metadata. Last definition wins on
-- replay, so this drop+recreate leaves the same end state as a fresh install.
-- ══════════════════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════════════════
-- Migration 107: repairs split per workspace rule (GVM-307) — mirror
-- ══════════════════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════════════════
-- Migration 108: repair payer + largest-remainder splits + repair validation
-- (GV-269) — mirror (function bodies byte-identical for the GV-175 equivalence
-- check; last definition wins on replay, so this drop+recreate leaves the same
-- end state as a fresh install running every migration).
-- ══════════════════════════════════════════════════════════════════════════
...
-- Verbatim from 109_notification_snooze_quiet_hours.sql: snooze_until (one-shot
-- pause) + quiet hours (recurring nightly window, Europe/Copenhagen, default 22->7)
-- on notification_preferences; get/set RPCs re-declared off 095 with the new
-- columns/params. Timezone math lives in the govehlo-web push filter; this stores
-- raw hours only.
...
-- Per-ledger body mirrors generate_due_recurring_expenses verbatim; iterates every ledger
-- with a due active template and materialises occurrences into that ledger's open period
-- with the same idempotence key, so client + scheduler never double-generate (GV-272).
...
comment on column public.ledger_members.last_seen_at is
  'Minimal activity metadata (GVM-66): the last time this member''s client sent a presence heartbeat (touch_member_presence). Null means never/unknown. Workspace-visible only, EU-hosted, nulled on account deletion by delete_my_account.';

-- touch_member_presence: the caller stamps their OWN membership's last_seen_at.
-- Resolves the actor via current_ledger_member_id (the same JWT-email -> active
-- member mapping every event-writing RPC uses), so a member can only ever bump
-- their own row. EVENT-FREE by design (no ledger_events insert). Columns are
-- qualified against the target_ledger_id param (ambiguity guard).
create or replace function public.touch_member_presence(target_ledger_id text)
returns void
language plpgsql
security definer
set search_path = public
as $$
declare
  v_member_id uuid := public.current_ledger_member_id(target_ledger_id);
begin
  if v_member_id is null then
    raise exception 'Caller is not an active member of this ledger' using errcode = '22023';
  end if;
...
revoke all on function public.touch_member_presence(text) from public;
grant execute on function public.touch_member_presence(text) to authenticated;

-- Deletion cleanup: re-declare delete_my_account off migration 096 (the newest
-- prior definition) with one addition — the member-scrub step also nulls
-- last_seen_at, so a deleted account leaves no residual activity metadata behind.
create or replace function public.delete_my_account()
returns jsonb
language plpgsql
security definer
...
insert into public.fuel_ledger_schema_migrations (migration_id, description)
values ('111_member_last_seen',
        'Persisted member last-seen (GVM-66): ledger_members gains last_seen_at (null = never/unknown); event-free touch_member_presence(target_ledger_id) lets a caller stamp their own membership heartbeat (validates active member, 22023) without writing ledger_events; delete_my_account re-declared off 096 to also null last_seen_at in the member scrub.')
on conflict (migration_id) do update
set description = excluded.description,
    applied_at = now();

-- ═══════════════════════════════════════════════════════════════════════════
-- Verbatim from 112_release_blockers_remediation.sql (GV-273/274/275). Function
-- bodies below are byte-identical to the migration (schema-equivalence guard).
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
-- Verbatim from 113_close_reminder_running_mode.sql (GVM-318). Function body
-- below is byte-identical to the migration (schema-equivalence guard).
-- ═══════════════════════════════════════════════════════════════════════════
