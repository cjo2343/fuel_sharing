...
guidelines/
  patterns-workspace-switcher.card.html  Workspace switcher pattern — 3 states
  …                                 (see full list above)
ui_kits/
  app/index.html                    Interactive GoVehlo PWA prototype (5 screens)
  admin/workspace-switcher.html     Interactive workspace switcher prototype
readme.md                           This file
...
The admin sidebar includes a **WorkspaceSwitcher** — a dropdown that lets admins switch between car-sharing groups (workspaces). It lives below the GoVehlo logo, above the nav items.

### Location
- **Integrated:** `templates/admin-shared/AdminLayout.js` — `WorkspaceSwitcher` component, rendered automatically in all admin templates
- **Prototype:** `ui_kits/admin/workspace-switcher.html` — standalone interactive demo with full admin layout context
- **Guideline card:** `guidelines/patterns-workspace-switcher.card.html` — visual reference showing Default, Hover, and Open states

### Design Spec
- **Trigger:** Full-width button in sidebar, `rgba(255,255,255,0.06)` resting → `0.10` hover → `0.12` open. 10px border-radius.
- **Avatar:** 28×28 rounded-square (8px radius) with two-letter group initials. Colors from brand palette: Forest `#2D6A4F`, Leaf `#52B788`, Amber `#F4A261`.
- **Dropdown:** Dark panel `#243A2A`, `1px solid rgba(255,255,255,0.10)`, 10px radius, elevated shadow `0 8px 32px rgba(0,0,0,0.3)`. Entrance animation: fade + slide up, 160ms ease-out.
- **Items:** 24×24 avatars, name + member count + role. Active workspace has leaf-green checkmark `#52B788`.
- **Create action:** Plus icon + "Create new group" at bottom, separated by `rgba(255,255,255,0.08)` divider.
- **Close:** Click outside dismisses. Chevron rotates 180° on open (160ms ease).

### Workspace Data Shape
```js
{ id: string, name: string, members: number, color: string, initial: string, role: 'Owner' | 'Admin' | 'Member' }
```

### Integration Notes
- `WorkspaceSwitcher` is exported to `window.WorkspaceSwitcher` from AdminLayout.js
- Currently manages its own state with sample data (3 workspaces). In production, wire `activeId` and `onSwitch` props to your routing/state layer.
- Amber avatar text uses `#1A2E1F` (deep forest) per the `--text-on-amber` token. Green avatars use `#D8F3DC` (mist).

---

## Admin Templates

Three admin templates share `templates/admin-shared/AdminLayout.js` for consistent chrome (sidebar with workspace switcher, top bar, avatar):
...
- **Workspace switcher uses sample data.** The 3 workspaces in AdminLayout.js are hardcoded for prototyping. Wire to real data when building the production admin.
